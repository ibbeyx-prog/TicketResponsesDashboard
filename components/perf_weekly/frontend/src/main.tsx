...
function ConnectedWeekly(props: { args: StreamlitArgs }) {
  const payload = normalizePayload(props.args?.data);
  const height = typeof props.args?.height === "number" ? props.args.height : 720;
...
const WrappedWeekly = withStreamlitConnection(ConnectedWeekly);
...
createRoot(rootEl).render(
  <StrictMode>{inStreamlit ? <WrappedWeekly /> : <App />}</StrictMode>,
);
